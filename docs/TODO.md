...
- [ ] command() ~500-line monolith — Step 1 done: extracted the shared "after any player action" block (split-card dealing, NPC auto-play, dealer-ready/bust-vote-window checks) into `_after_player_action(game_session)`, called from the digital hit/stand/double/split branch. Remaining options for further splitting: (a) a dict-based command registry mapping cmd -> handler_fn(session, parts, ...), each handler in its own function; (b) split into per-mode dispatcher functions (_handle_digital_command, _handle_referee_command). Full registry refactor is higher effort/risk and worth doing only if you're adding more commands soon.
