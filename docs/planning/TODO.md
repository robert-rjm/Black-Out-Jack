...
- [ ] simplified ruleset
  ...
  - [ ] have feature to select each additional rule (start with simplified and then able to add 1 rule at a time)
- [ ] data export
  - potentially .pdf file output with nicer graphs
  - show in Dealer who drank most for each ace
  - show luckiest hits
  - for Milestone show Deltas to next best players
- [X] Targeted Drinking: target individual players to drink
