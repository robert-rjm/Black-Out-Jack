...
- [ ] **1. Fix bust-vote side-bet mismatch** — make `apply_bust_vote_penalties` (`drink_tracker.py:149`) and `payout_tracker.py` read each player's own `_pbets` stake instead of the global `session.bet_amount`, matching the lookup already used at the deduct site (`polling.py:642`). Add a `tests/app/test_payout_tracker.py` case with a custom per-player bet.
- [ ] **2. Guard Dealer Lottery deck exhaustion** — make `resolve_dealer_lottery`'s `Deck()` reshuffle-in-place (or start from 2+ decks) so `_deal_and_resolve_hand`'s `deck.cards.pop()` can't run dry. Add a forced-re-split test in `tests/app/test_dealer_lottery.py`.
- [ ] **3. Confirm/pin Render worker count** — verify the Render dashboard start command uses `--workers 1`; consider committing a `Procfile` so it's enforced in-repo. No code change if already correct — just confirm.
- [ ] **4. Add a lock around registration/seat-transfer mutations** — one `threading.Lock` per `GameRoom` around the check-then-append sequences in `polling.py:118-161` and `admin.py:341-350`, cheap insurance regardless of what #3 finds.
- [ ] **5. Cap/rotate session-lifetime lists** — trim or roll off `session.drinks.csv_rows`, `_decision_log`, `_dealer_lottery_decision_log` for very long sessions (e.g. keep last N rounds' verbose fields, or drop `visible_cards`/`hand_cards_before` after export).
- [ ] **6. Correct `Improvements.md` item 5's status** — rewrite from "Large effort, not started" to reflect the actual state (digital path extracted into `game_engine.py`; referee/digital still parallel; the merge itself not done), so the Busfahrer prerequisite question stays accurate for whoever picks it up next.
...
- [ ] **10. (Optional, low urgency) Start on Frontend component architecture Option A** (class-based JS components, no framework) from `Improvements.md` item 7, closing out the FC-1 `onclick=`-string fragility for good rather than just relying on `escapeHtml()`/`sanitize_name()` as a safety net.
