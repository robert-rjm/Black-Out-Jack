...
- [~] **10. Frontend component architecture Option A** (class-based JS components, no framework) from `Improvements.md` item 7 — in progress. Converted so far, each as a `mount(el)`/`render(state)` class with one delegated click listener replacing per-render `onclick=` strings: `DrinksPanel` (round/drinks pane, [table.js](../../static/js/ui/table.js)), `BustGivePanel` and `DealerLotteryGivePanel` (handout give-panels, [admin.js](../../static/js/ui/admin.js)), and `PendingRegBanner` (registration accept/deny banner — the one site that had server data, `client_id`, interpolated directly into an `onclick=` string rather than via `dataset`). Remaining `onclick=` sites (`kpi.js`, `lobby.js`, `trivia.js`) carry no user/server-derived data, so they're lower priority. Not yet converted: the bust-vote status/modal/timer cluster (`updateBustVoteUI` and friends, `admin.js`) — larger and stateful (countdown resync, auto-pass-on-timeout), deliberately deferred to its own pass rather than risking a regression in the same session as the give-panel conversions.
