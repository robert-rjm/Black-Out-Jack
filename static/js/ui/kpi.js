...
// Compares a live-session percentage/average against the simulated baseline
// in BENCHMARKS (static/js/benchmarks.js, generated by `python
// scripts/simulation.py`). Live sessions are tiny samples (tens of rounds)
// vs. the 10,000-round baseline, so bands are deliberately wide:
//   within 25% of baseline -> neutral (no special color)
//   25-50% off             -> yellow
//   >50% off               -> green/red, depending on direction
...
function benchmarkColor(value, benchmark, { lowerIsBetter = false } = {}) {
  if (value === null || value === undefined) return "";
  if (!benchmark) return ""; // BENCHMARKS not generated yet — no baseline to compare to
  const diffPct = ((value - benchmark) / benchmark) * 100;
  const better  = lowerIsBetter ? diffPct < 0 : diffPct > 0;
  const abs     = Math.abs(diffPct);
  if (abs <= 25) return "";
  if (abs <= 50) return "color:var(--yellow)";
...
function renderStats(state) {
  ...
  const tbody = rows.map(r => {
    ...
    const avgSipsCell = r.avgSips !== "—"
      ? `<span style="${benchmarkColor(parseFloat(r.avgSips), _perPlayerSipBenchmark, { lowerIsBetter: true })}">${r.avgSips}</span>`
      : r.avgSips;
  ...
  if (totalBJ > 0) {
    ...
    const bjCol     = bjRate !== null ? benchmarkColor(bjRate, bjBench) : "";
  ...
  if (dealerBustPct !== null) {
    ...
    const col      = dealerBustRnds === 0 ? "" : benchmarkColor(dealerBustPct, dbBench);
